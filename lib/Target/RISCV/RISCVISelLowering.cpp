...
SDValue RISCVTargetLowering::lowerShiftLeftParts(SDValue Op,
  ...
  SDValue MinusXLen = DAG.getConstant(-Subtarget.getXLen(), DL, VT);
  ...
  SDValue ShiftRightLo = DAG.getNode(ISD::SRL, DL, VT, ShiftRight1Lo,
                                     XLenMinus1Shamt);
...
SDValue RISCVTargetLowering::lowerShiftRightParts(SDValue Op,
                                                  SelectionDAG &DAG,
                                                  bool IsSRA) const {
  SDLoc DL(Op);
  SDValue Lo = Op.getOperand(0);
  ...
  SDValue ShiftLeftHi = DAG.getNode(ISD::SHL, DL, VT, ShiftLeftHi1,
                                    XLenMinus1Shamt);
  SDValue LoTrue = DAG.getNode(ISD::OR, DL, VT, ShiftRightLo, ShiftLeftHi);
  SDValue HiTrue = DAG.getNode(ShiftRightOp, DL, VT, Hi, Shamt);
  SDValue LoFalse = DAG.getNode(ShiftRightOp, DL, VT, Hi, ShamtMinusXLen);
  SDValue HiFalse = IsSRA ? DAG.getNode(ISD::SRA, DL, VT, Hi, XLenMinus1) :
                            Zero;
...
static MachineBasicBlock *emitSelectPseudo(MachineInstr &MI,
  ...
  auto SequenceMBBI = MachineBasicBlock::iterator(MI);
  MachineInstr *LastSelectPseudo = &MI;

  do {
    SequenceMBBI = skipDebugInstructionsForward(SequenceMBBI, BB->end());
    if (SequenceMBBI == BB->end())
      break;
